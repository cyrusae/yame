...
pub fn render_search_bar(f: &mut Frame, area: Rect, app: &App) {
    ...
    let label = if search.regex_mode {
        " Search [re]: "
    } else {
        " Search: "
    };
    ...
    if search.show_replace && area.height >= 2 {
        ...
        for ch in replace_display.chars() {
            if rx >= area.x + area.width || rx >= hint_x {
                break;
            }
            buf[(rx, replace_y)]
                .set_char(ch)
